...
import {
    auditEntryManagerRuntime as auditEntryManagerRuntimeImpl,
    getEntryManagerRuntimeSnapshot as getEntryManagerRuntimeSnapshotImpl,
} from './entry-manager-runtime.js';
...
export const FACT_EXTRACTION_PROMPT = [
    ...
    'Use the Current In-World Time block below if available. Treat it as the default temporal anchor unless the exchange clearly advances time beyond it.',
    'If the block includes a Day and/or Date, carry those into the fact timestamp whenever the exchange does not explicitly contradict them.',
    'Do not drop known Day/Date information and replace it with vague labels like "evening" or "later" by themselves.',
    'Write "unknown" only if there are no time cues at all.',
    '',
    KEYWORD_RULES,
    '',
...
function getBookEpoch(bookName) {
    return entryManagerRuntimeState.bookEpochs.get(bookName) || 0;
}

function bumpBookEpoch(bookName) {
    if (typeof bookName !== 'string' || !bookName.trim()) return;
    entryManagerRuntimeState.bookEpochs.set(bookName, getBookEpoch(bookName) + 1);
}
...
export async function getCachedWorldInfo(bookName) {
    if (entryManagerRuntimeState.worldInfoCache.has(bookName)) return entryManagerRuntimeState.worldInfoCache.get(bookName);
    const data = await loadWorldInfo(bookName);
    if (data) {
        entryManagerRuntimeState.worldInfoCache.set(bookName, data);
        entryManagerRuntimeState.cachedBookEpochs.set(bookName, getBookEpoch(bookName));
    }
    return data;
}
...
export function getCachedWorldInfoSync(bookName) {
    return entryManagerRuntimeState.worldInfoCache.get(bookName) || null;
}
...
export function invalidateWorldInfoCache(bookName) {
    if (bookName) {
        entryManagerRuntimeState.worldInfoCache.delete(bookName);
        entryManagerRuntimeState.cachedBookEpochs.delete(bookName);
        entryManagerRuntimeState.dirtyBooks.add(bookName);
        bumpBookEpoch(bookName);
    } else {
        entryManagerRuntimeState.worldInfoCache.clear();
        entryManagerRuntimeState.dirtyBooks.clear();
        entryManagerRuntimeState.bookEpochs.clear();
        entryManagerRuntimeState.cachedBookEpochs.clear();
    }
}
...
export function invalidateDirtyWorldInfoCache() {
    for (const bookName of entryManagerRuntimeState.dirtyBooks) {
        entryManagerRuntimeState.worldInfoCache.delete(bookName);
        entryManagerRuntimeState.cachedBookEpochs.delete(bookName);
    }
    entryManagerRuntimeState.dirtyBooks.clear();
...
export function getEntryManagerRuntimeSnapshot() {
    return getEntryManagerRuntimeSnapshotImpl();
}

export function auditEntryManagerRuntime(snapshot = getEntryManagerRuntimeSnapshot()) {
    return auditEntryManagerRuntimeImpl(snapshot);
}
