...
import {
  PREWARM_CACHE_MAX_AGE_MS,
  buildPreWarmCacheKey,
  isPreWarmCacheFresh,
  getSmartContextRuntimeSnapshot as getSmartContextRuntimeSnapshotImpl,
  auditSmartContextRuntime as auditSmartContextRuntimeImpl,
} from "./smart-context-runtime.js";
import {
  smartContextRuntimeState,
  setSmartContextRuntimeSnapshotState,
} from "./smart-context-runtime-state.js";
import {
  shuffleArray,
  isActSummaryEntry,
  isStorySummaryEntry,
...
export function invalidatePreWarmCache() {
  smartContextRuntimeState.preWarmEpoch += 1;
  smartContextRuntimeState.preWarmedCandidates = null;
  smartContextRuntimeState.preWarmCacheKey = null;
  smartContextRuntimeState.preWarmSource = "smart-context";
  smartContextRuntimeState.preWarmCachedAt = 0;
  smartContextRuntimeState.cachedPreWarmEpoch = 0;
  smartContextRuntimeState.preWarmInFlight = false;
  smartContextRuntimeState.preWarmPendingKey = null;
  smartContextRuntimeState.preWarmPendingEpoch = 0;
  smartContextRuntimeState.lastReportedPreWarmKey = null;
  smartContextRuntimeState.lastReportedInjectionKey = null;
  _derivedKeyCache.clear();
}

export function getSmartContextRuntimeSnapshot() {
  return getSmartContextRuntimeSnapshotImpl({
...
export function auditSmartContextRuntime(snapshot = getSmartContextRuntimeSnapshot()) {
  return auditSmartContextRuntimeImpl(snapshot);
}
...
export function processRelevanceFeedback() {
  if (smartContextRuntimeState.lastInjectedEntries.length === 0) return;
  ...
  for (const entry of smartContextRuntimeState.lastInjectedEntries) {
  ...
  smartContextRuntimeState.lastInjectedEntries = [];
}
...
function reportSmartContextSelections(selectedEntryInfo, source = "smart-context") {
  ...
  if (injectionKey === smartContextRuntimeState.lastReportedInjectionKey) return;
  smartContextRuntimeState.lastReportedInjectionKey = injectionKey;
...
function reportPreWarmCandidates(candidates, cacheKey, source = "smart-context") {
  ...
  if (!cacheKey || cacheKey === smartContextRuntimeState.lastReportedPreWarmKey) return;
  smartContextRuntimeState.lastReportedPreWarmKey = cacheKey;
...
export function buildSmartContextPrompt(options = {}) {
  ...
  if (isPreWarmCacheFresh(cacheKey)) {
    candidates = smartContextRuntimeState.preWarmedCandidates;
    selectionSource = smartContextRuntimeState.preWarmSource || "smart-context";
  } else {
  ...
  if (applySideEffects) {
    smartContextRuntimeState.lastInjectedEntries = selectedEntryInfo;
...
export async function preWarmSmartContext({ source = 'smart-context' } = {}) {
  ...
  const startEpoch = smartContextRuntimeState.preWarmEpoch;
  const requestId = ++smartContextRuntimeState.preWarmRequestId;
  smartContextRuntimeState.preWarmInFlight = true;
  smartContextRuntimeState.preWarmPendingKey = cacheKey;
  smartContextRuntimeState.preWarmPendingEpoch = startEpoch;
  ...
  try {
    ...
    if (
      requestId !== smartContextRuntimeState.preWarmRequestId
      || smartContextRuntimeState.preWarmEpoch !== startEpoch
      || buildPreWarmCacheKey() !== cacheKey
    ) {
      return;
    }

    smartContextRuntimeState.preWarmedCandidates = candidates;
    smartContextRuntimeState.preWarmCacheKey = cacheKey;
    smartContextRuntimeState.preWarmSource = source;
    smartContextRuntimeState.preWarmCachedAt = Date.now();
    smartContextRuntimeState.cachedPreWarmEpoch = smartContextRuntimeState.preWarmEpoch;
    reportPreWarmCandidates(candidates, cacheKey, source);
  } finally {
    if (requestId === smartContextRuntimeState.preWarmRequestId) {
      smartContextRuntimeState.preWarmInFlight = false;
      smartContextRuntimeState.preWarmPendingKey = null;
      smartContextRuntimeState.preWarmPendingEpoch = 0;
    }
...
export const __smartContextDebug = {
  ...
  setRuntimeSnapshotState: setSmartContextRuntimeSnapshotState,
